...
bool TYPING_HEATMAP(effect_params_t* params) {
    ...
    if (params->init) {
        rgb_matrix_set_color_all(64, 64, 64);
    ...
    for (int i = led_min; i < led_max; i++) {
        ...
        for (uint8_t j = 0; j < led_count; ++j) {
            ...
            if (j == 28) {
                if (rgb.r > 200) rgb.r = 200;
                if (rgb.g > 200) rgb.g = 200;
                if (rgb.b > 200) rgb.b = 200;
            }
        ...
        if (decrease_heatmap_values) {
            ...
            if (g_rgb_frame_buffer[row][col] < 64) {
                g_rgb_frame_buffer[row][col] = 64;
            }
        }
    }
